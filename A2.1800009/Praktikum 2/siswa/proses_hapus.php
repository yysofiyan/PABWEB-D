<?php 
	...
	if (isset($_GET['nis'])) {
		...
		$query = mysqli_query($db, "SELECT foto FROM tb_siswa WHERE nis='$nis'") or die('Ada kesalahan pada query tampil data foto :'.mysqli_error($db));
		...
		if ($hapus_file) {
			...
			$delete = mysqli_query($db, "DELETE FROM tb_siswa WHERE nis='$nis'") or die('Ada kesalahan pada query delete :'.mysqli_error($db));
			// cek hasil query
			if ($delete) {
