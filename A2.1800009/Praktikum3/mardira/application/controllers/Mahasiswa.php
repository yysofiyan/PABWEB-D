...
class Mahasiswa extends CI_Controller
{
    ...
    {
        $attr_nim = array(
            'requered' => 'NIM Harus Diisi !!!',
            'min_length' => 'NIM Minimal 8 Karakter !!!',
            'max_length' => 'NIM Melebihi 8 Karakter !!!',
            'numeric' => 'NIM tidak menggunakan huruf !!!',
        );
        $attr_nama = array (
            'requered' => 'Nama Harus Diisi !!!',
            'min_length' => 'Nama Mahasiswa Minimal 15 Karakter !!!',
            'max_length' => 'Nama Mahasiswa Maksimal 30 Karakter !!!',
        );
        $this->form_validation->set_rules('nim', 'NIM', 'trim|required|numeric|min_lenght[8]|max_lenght[8]', $attr_nim);
        $this->form_validation->set_rules('nama_mhs', 'Nama Mahasiswa', 'trim|required|numeric|min_lenght[5]|max_lenght[50]', $attr_nama);
    }
    public function tambah_aksi()
    {
        ...
        if ($validasi == FALSE){
            ...
            $nama_mhs = $this->input->posy('nama_mhs');
    ...
    public function edit($id)
    {
        ...
        if ($get_row->num_rows() > 0) {
            ...
            $attr_id = array(
                'type' => 'hidden',
                'name' => 'id_mahasiswa',
                'attr' => set_value('id_mahasiswa', $id_mahasiswa)
            );
            $nim = $row->nim;
            ...
            $input_nama = form_input('Nama Mahasiswa/ $nama_mhs');
            $input_id = form_input($attr_id);
            $get_prodi = $this->Mahasiswa_model->get_prodi();
            $prodi = array();
            foreach ($get_prodi as $r) {
