...
if (isset($_POST['simpan'])) {
	...
	$tanggal_lahir	= mysqli_real_escape_string($db, trim(date('Y-m-d', strtotime($_POST['tgl_lahir']))));
	...
	$query	= mysqli_query($db, "SELECT nis FROM tabel_siswa WHERE nis='nis'")
	...
	if ($rows > 0) {
		...
		if(move_uploaded_file($tmp_file, $path)) {
			...
			$insert	= mysqli_query($db, "INSERT INTO tabel_siswa(nis,nama,tempat_lahir,tgl_lahir,jenis_kelamin,agama,alamat,no_hp,foto)
